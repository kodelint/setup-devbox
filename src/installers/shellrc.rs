...
// `contains_multiline_block`: Checks if a block of lines already exists in the RC file.
...
/// It takes your desired shell configurations (`raw_configs` and `aliases`)
...
///     inside that RC file. This is crucial for avoiding duplicates and
///     preserving user's existing configurations.
/// 3.  **Compare and collect new configs**: It meticulously checks your
///     `raw_configs` (multi-line snippets of shell commands) and `aliases`
///     from your `shellrc.yaml` against what's already in the RC file.
///     Only truly *new* or *missing* entries get selected to be added.
...
///              It tells us which shell to target (e.g., "zsh", "bash") and
///              provides raw configuration snippets (`raw_configs`).
/// * `aliases`: A list (`&[AliasEntry]`) of specific command shortcuts (aliases)
///              you want to set up in your shell. This is a slice because it's
///              a read-only view of a collection of `AliasEntry` structs.
pub fn apply_shellrc(shellrc: &ShellRc, aliases: &[AliasEntry]) {
    ...
    // as it shows the exact input to this function.
    ...
    // The `get_rc_file` helper function does this detective work for us based on the shell name.
    let Some(rc_path) = get_rc_file(&shellrc.shell) else {
        // If `get_rc_file` comes up empty (returns `None`), it means the shell
        // you've specified isn't one we currently know how to handle (only zsh and bash are supported).
        // We'll log a warning and gracefully stop this operation for this unsupported shell.
        log_warn!(
            "[Shell Config] Skipping shell configuration: Unsupported or unrecognized shell '{}'. Our current mission only covers 'zsh' and 'bash'.",
            shellrc.shell.red() // Highlight the unsupported shell name in red.
        );
    ...
    // returning a `Vec<String>` where each string is a line from the file.
    let existing_lines = read_rc_lines(&rc_path);
    // A quick debug check to confirm how many lines we successfully read from the RC file.
    log_debug!(
    ...
    for raw_config_entry in &shellrc.raw_configs {
        ...
        // We filter out empty lines and comment lines (starting with '#') to normalize the input
        // for `contains_multiline_block`, similar to how `read_rc_lines` might process.
        let raw_config_lines: Vec<String> = raw_config_entry
            .lines() // Iterate over lines in the multiline string.
            .map(|s| s.to_string()) // Convert &str to String.
            .filter(|s| !s.trim().is_empty() && !s.trim().starts_with('#')) // Remove empty or comment-only lines.
            .collect(); // Collect into a new vector of strings.
        ...
        // Log the first line of the raw config entry for debugging, dimming it to indicate it's an internal detail.
        log_debug!(
            "[Shell Config] Examining raw config entry ({} lines): '{}'",
            raw_config_lines.len().to_string().dimmed(),
            raw_config_lines[0].trim().dimmed() // Just show the first line for debug context.
        );

        // Check if the entire block of raw configuration lines already exists in the RC file.
    ...
    // These are typically single-line `alias` commands.
    log_debug!("[Shell Config] Next up: let's look at your custom 'aliases' from shellrc.yaml.");
    // Now, we do a similar check for each `AliasEntry` you've defined.
    ...
    if new_lines_to_add.is_empty() {
        ...
        log_info!(
            ...
            rc_path.display().to_string().cyan() // Display the path, colored.
        );
    } else {
        // Ah, there are new lines to add! Let's get to work.
        // Log the number of new lines and the target file path.
        log_info!(
        ...
        match append_to_rc_file(&rc_path, new_lines_to_add) {
            ...
            Err(err) => {
                ...
                // We'll alert the user with a warning, suggesting they check permissions and try again.
                log_warn!(
